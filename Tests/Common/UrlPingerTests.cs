...
namespace Tests.Common
{
    ...
    new[] { "http://[2001:db8::1]:8080/path" })]
        [Arguments("International domain http://中国移动.中国",
    new[] { "http://中国移动.中国" })]
    ...
    new[] { "https://example.com/file%20name.txt" })]
        [Arguments("Underscore in domain http://my_site.example.com",
    new[] { "http://my_site.example.com" })]
    ...
    new[] { "https://my-site.example.org" })]
        ...
        [Arguments("URL ending with dot Visit https://example.com.",
    new[] { "https://example.com" })] // Assuming trailing dot is excluded
    ...
    new[] { "https://ex.co/a,b,c.html" })]
      ...
      new[] { "https://line1.com", "https://line2.com" })]
        ...
        [Arguments("URL with Unicode characters https://例子.测试", new[] { "https://例子.测试" })]
        [Arguments("URL with URL-encoded characters https://example.com?q=%20", new[] { "https://example.com?q=%20" })]
        [Arguments("ftp://ftp.example.com", new[] { "ftp://ftp.example.com" })]
        [Arguments("ftps://ftp.example.com", new[] { "ftps://ftp.example.com" })]
        [Arguments("sftp://sftp.example.com", new[] { "sftp://sftp.example.com" })] // Assuming you want to treat sftp like a typical URL
        [Arguments("mailto:test@example.com", new[] { "mailto:test@example.com" })]
        [Arguments("file:///path/to/file", new[] { "file:///path/to/file" })]
        [Arguments("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAUAAAAFCAYAAACNbyblAAAAHElEQVQI12P4//8/w+bAAdQDgWEm0/0AAAAASUVORK5CYII=", new[] { "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAUAAAAFCAYAAACNbyblAAAAHElEQVQI12P4//8/w+bAAdQDgWEm0/0AAAAASUVORK5CYII=" })] //Data URI
        [Arguments("ws://example.com/socket", new[] { "ws://example.com/socket" })]
        [Arguments("wss://example.com/socket", new[] { "wss://example.com/socket" })]
        [Arguments("gopher://gopher.example.com", new[] { "gopher://gopher.example.com" })]
        ...
        [Arguments("IP address without protocol 192.168.1.1", new string[0])] //Shouldn't match.
        [Arguments("IP address with port without protocol 192.168.1.1:8080", new string[0])] //Shouldn't match.
        [Arguments("IP address surrounded by text something192.168.1.1something", new string[0])] // more edge cases
        ...
        [Arguments("Url followed by punctuation. https://punctuation.com;", new[] { "https://punctuation.com" })]
        [Arguments("Url followed by punctuation. https://punctuation.com)", new[] { "https://punctuation.com" })]
        [Arguments("Nested URL-like strings inside a URL https://example.com/path?q=http://nested.com", new[] { "https://example.com/path?q=http://nested.com" })] //Edge Case
        [Arguments("Mixed case and weird characters HttPs://ExAmPlE-.com", new[] { "https://example-.com" })]
        [Arguments("Url with underscore https://under_score.com", new[] { "https://under_score.com" })]
