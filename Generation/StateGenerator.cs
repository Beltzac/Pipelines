        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            context.RegisterSourceOutput(syntaxProvider, (context, tuple) =>
            {
                ...
                try
                {
                    ...
                    // Check if the class implements ITracksLoading
                    var implementsTracksLoading = tuple.symbol.AllInterfaces.Any(i => i.Name == "ITracksLoading");

                    var source = GenerateStateServiceSource(namespaceName, className, serviceName, properties, implementsTracksLoading);
...
{registrations}
        ...
        private static string GenerateStateServiceSource(string namespaceName, string className, string serviceName, List<IPropertySymbol> properties, bool implementsTracksLoading)
        {
        ...
        {{
            ...
            var loadingScopeMethod = implementsTracksLoading ? @"
        /// <summary>
        /// Begins a loading scope that automatically manages the loading state.
        /// </summary>
        /// <returns>An IDisposable that ends the loading state when disposed.</returns>
        public IDisposable BeginLoadingScope()
        {
            return new LoadingScope(_state);
        }" : "";

            var usings = implementsTracksLoading ? @"using System;
using System.Collections.Generic;
...
{{
    ...
    {{
        ...
        public {className} GetState() => _state;{loadingScopeMethod}
